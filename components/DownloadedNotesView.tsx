...
// Fix: Replaced DownloadedNote with the correct type DownloadedItem.
import { View, DownloadedItem } from '../types';

// Storage logic
const STORAGE_KEY = 'padhlo-downloaded-notes';

const getNotes = (): DownloadedItem[] => {
    try {
...
const removeNote = (noteId: string): DownloadedItem[] => {
...
const DownloadedNotesView: React.FC<DownloadedNotesViewProps> = ({ setView }) => {
    const [notes, setNotes] = useState<DownloadedItem[]>([]);

    useEffect(() => {
        setNotes(getNotes());
    }, []);

    const handleNoteClick = (note: DownloadedItem) => {
        setView({
            name: 'chapter',
    ...
    const noteItems = notes.filter(item => item.type === 'note');

    const groupedNotes = noteItems.reduce((acc: Record<string, DownloadedItem[]>, note) => {
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-200 mb-6">Downloaded Notes</h2>
            ...
            {noteItems.length === 0 ? (
                ...
                <div className="space-y-8">
                    {/* Fix: Explicitly typed the destructured array from Object.entries to ensure `notesInGroup` is correctly typed as DownloadedItem[]. */}
                    {Object.entries(groupedNotes).map(([groupName, notesInGroup]: [string, DownloadedItem[]]) => (
                        <div key={groupName}>
